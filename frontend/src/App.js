...
const MusicProvider = ({ children }) => {
  ...
  const [tracks, setTracks] = useState([
    { id: 1, name: "versace", url: "/assets/versace.mp3", artist: "Bruno Mars" },
    { id: 2, name: "anugerah", url: "/assets/anugerah.mp3", artist: "Andmesh" },
    { id: 3, name: "delo", url: "/assets/delo.mp3", artist: "Delosound" },
    { id: 4, name: "gensan", url: "/assets/gensan.mp3", artist: "Gensanmaier" },
    { id: 5, name: "forget", url: "/assets/forget.mp3", artist: "Music for Video" },
    { id: 6, name: "sonican", url: "/assets/sonican.mp3", artist: "Sonican" },
    { id: 7, name: "xethrocc", url: "/assets/xethrocc.mp3", artist: "Xethrocc" }
  ]);
...
const MusicVisualizer = ({ isPlaying }) => {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (canvasRef.current) {
        const bars = canvasRef.current.children;
        for (let i = 0; i < bars.length; i++) {
          const val = dataArray[i % dataArray.length] / 255.0;
          const height = Math.max(2, val * 24);
  ...
  return (
    <div ref={canvasRef} className="flex items-end justify-center gap-[2px] h-6 w-full opacity-60">
      {[...Array(80)].map((_, i) => (
        <div
          key={i}
          className="w-1 bg-royal-purple/80 dark:bg-white/60 rounded-t-full shrink-0 transition-[height] duration-75"
          style={{ height: '2px' }}
        />
      ))}
    </div>
    ...
    <>
      <motion.header initial={{ y: -100 }} animate={{ y: 0 }} className={"fixed top-0 left-0 right-0 z-40 transition-all duration-500 " + (scrolled ? 'glass shadow-lg py-2' : 'bg-transparent py-4')}>
        <nav className="container mx-auto px-4 relative">
          ...
          {/* Real Audio Wave Visualizer on Navbar Border */}
          <div className="absolute bottom-0 left-0 right-0 translate-y-[2px] opacity-80 pointer-events-none">
            <MusicVisualizer isPlaying={isPlaying} />
          </div>
        </nav>
      </motion.header>

      {/* Minimalist Left Side Volume Control */}
      <motion.div
      ...
      >
        <motion.div
          className="bg-black/60 backdrop-blur-2xl border border-white/10 rounded-full shadow-2xl overflow-hidden flex items-center p-1.5"
          animate={{ height: volumeHover ? '220px' : '48px', width: '48px', flexDirection: 'column' }}
          transition={{ type: 'spring', stiffness: 400, damping: 30 }}
        >
          <div className="w-9 h-9 flex items-center justify-center shrink-0 cursor-pointer text-white hover:text-royal-purple transition-colors">
            {volume === 0 ? <VolumeX className="w-4 h-4" /> : <Volume2 className={"w-4 h-4 " + (isPlaying ? "text-royal-purple" : "")} />}
          </div>
          ...
          <AnimatePresence>
            {volumeHover && (
              <motion.div
                ...
                exit={{ opacity: 0, y: 10 }}
                className="flex flex-col items-center gap-4 pb-2 mt-4 flex-1"
              >
                <div className="h-24 flex items-center">
                  <input
                    ...
                    className="w-24 -rotate-90 origin-center h-1 bg-white/20 rounded-full accent-royal-purple cursor-pointer appearance-none"
                ...
                <button
                  ...
                  className="w-8 h-8 rounded-full bg-royal-purple/20 hover:bg-royal-purple text-white flex items-center justify-center transition-all"
                >
...
const MusicManager = () => {
  ...
  const handleAddTrack = () => {
    ...
    addTrack({ name: newTrackName.toLowerCase(), url, artist: 'Custom Admin' });
    setNewTrackName('');
    ...
    <Card className="border-0 shadow-card">
      ...
      <CardContent className="space-y-6">
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input value={newTrackName} onChange={(e) => setNewTrackName(e.target.value)} placeholder="e.g. favorite lofi" />
            </div>
            <div className="space-y-2">
        ...
        <div className="space-y-2">
          ...
          <div className="border rounded-xl divide-y dark:border-white/10 overflow-hidden">
            {tracks.map((track) => (
              <div key={track.id} className="p-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                <div className="flex items-center gap-4 flex-1">
                  <div className="w-10 h-10 rounded-full bg-royal-purple/10 flex items-center justify-center text-royal-purple"><Music className="w-5 h-5" /></div>
                  <div className="flex-1">
                    <Input
                      value={track.name}
                      onChange={(e) => updateTrack(track.id, { name: e.target.value.toLowerCase() })}
                      className="h-8 border-transparent hover:border-royal-purple/20 focus:border-royal-purple bg-transparent font-medium p-0"
                    />
                    <p className="text-[10px] text-muted-foreground uppercase tracking-widest">{track.artist}</p>
                  </div>
                </div>
                <Button variant="ghost" size="icon" onClick={() => removeTrack(track.id)} className="text-red-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-full"><Trash2 className="w-4 h-4" /></Button>
              </div>
            ))}
          </div>
        </div>
