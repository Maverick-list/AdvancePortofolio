...
const AnimatedLogo = () => (
  ...
  >
    <motion.div 
      className="w-10 h-10 rounded-xl gradient-bg flex items-center justify-center shadow-glow-purple"
      animate={{ 
        boxShadow: [
          "0 0 20px rgba(106, 0, 255, 0.4)",
          "0 0 30px rgba(255, 94, 207, 0.4)",
          "0 0 20px rgba(106, 0, 255, 0.4)"
        ]
      }}
...
const FloatingAIAgent = () => {
  ...
  const fetchSuggestions = async () => {
    try {
      // Try to get suggestions from AI
      const response = await api.get('/ai/suggestions', token);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { 
        role: 'assistant', 
        content: "Hello! I'm your AI assistant. I can help you manage tasks, set reminders, and stay organized. How can I assist you today?" 
      }]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      ...
      >
        {/* Particles Ring */}
        <motion.div
          className="absolute inset-0 ai-particles-ring"
          style={{ width: 80, height: 80, marginLeft: -8, marginTop: -8 }}
        >
          {[...Array(6)].map((_, i) => (
            <motion.div
              key={i}
              className="absolute w-2 h-2 rounded-full bg-gradient-to-r from-royal-purple to-hot-pink"
              style={{
                top: '50%',
                left: '50%',
        ...
        {/* Glow Layer */}
        <motion.div
          className="absolute inset-0 rounded-full ai-glow-layer"
          style={{
            background: 'radial-gradient(circle, rgba(184, 77, 255, 0.4) 0%, transparent 70%)',
            width: 100,
            height: 100,
            marginLeft: -18,
            marginTop: -18,
        ...
        <motion.button
          ...
          className="relative w-16 h-16 rounded-full gradient-bg flex items-center justify-center ai-agent-orb cursor-pointer"
            ...
            <Card className="border-0 shadow-2xl overflow-hidden" style={{
              ...
              <div className="p-4 gradient-bg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-white">AI Personal Assistant</h3>
                ...
                {messages.length === 0 ? (
                  ...
                  >
                    <motion.div
                      ...
                      transition={{ duration: 3, repeat: Infinity }}
                    >
                      <Bot className="w-16 h-16 mx-auto mb-4 text-royal-purple/30" />
                    </motion.div>
                    <p className="text-muted-foreground text-sm">
                      Hi! I'm your AI assistant.<br/>
                      I can help with tasks, reminders, and more!
                    </p>
                    <div className="mt-4 space-y-2">
                      {["What's on my schedule?", "Create a reminder", "Help me stay focused"].map((q, i) => (
                        <button
                          key={i}
                          onClick={() => { setInput(q); }}
                          className="block w-full text-left text-sm px-3 py-2 rounded-lg bg-gradient-to-r from-purple-50 to-pink-50 hover:from-purple-100 hover:to-pink-100 transition-colors text-purple-700"
                        >
                          {q}
                        </button>
                      ))}
                      ...
                      >
                        <div className={\`max-w-[85%] p-3 rounded-2xl \${
                          msg.role === 'user'
                            ...
                            : 'bg-white shadow-md rounded-bl-sm border border-purple-100'
                        }\`}>
              ...
              <div className="p-4 border-t bg-gradient-to-r from-purple-50/50 to-pink-50/50">
                ...
                >
                  <Input
                    ...
                    className="flex-1 border-purple-200 focus:border-royal-purple"
                    disabled={loading}
                  />
                  <Button 
                    type="submit" 
          ...
          <div className="hidden md:flex items-center gap-6">
            {navLinks.map((link, index) => (
              ...
              >
                <Link
                  to={link.href}
                  className="flex items-center gap-2 text-foreground/70 hover:text-foreground transition-all font-medium hover:scale-105"
                >
...
const ArticlesPage = () => {
  ...
  return (
    <motion.div
      ...
      animate="animate"
      exit="exit"
      className="min-h-screen pt-24 pb-12 gradient-bg-page"
    >
      <div className="container mx-auto px-4">
        <motion.div
          initial={{ opacity: 0, y: 30 }}
        ...
        {loading ? (
          ...
          <Card className="max-w-md mx-auto text-center py-12 card-hover">
            <CardContent>
...
const ArticlePage = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen pt-24 flex items-center justify-center gradient-bg-page">
        <motion.div
          animate={{ rotate: 360 }}
          transition={{ duration: 2, repeat: Infinity, ease: "linear" }}
          className="w-12 h-12 rounded-full border-4 border-royal-purple border-t-hot-pink"
        />
      </div>
    );
  }
  ...
  if (!article) {
    return (
      <div className="min-h-screen pt-24 flex items-center justify-center gradient-bg-page">
        <Card className="max-w-md text-center p-8 card-hover">
        ...
        <div className="border-t pt-8">
          ...
          <div className="space-y-4">
            {article.comments?.map((c, i) => (
              <Card key={c.id || i} className="card-hover">
                <CardContent className="pt-4">
  ...
  return (
    ...
    >
            ...
            <CardHeader>
              <div className="flex items-center gap-2">
                <motion.div 
                  ...
                  animate={{ 
                    boxShadow: [
                      "0 0 10px rgba(106, 0, 255, 0.3)",
                      "0 0 20px rgba(255, 94, 207, 0.4)",
                      "0 0 10px rgba(106, 0, 255, 0.3)"
                    ]
                  }}
                  transition={{ duration: 2, repeat: Infinity }}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { to: '/admin/writing', icon: FileText, title: 'Write New Article', desc: 'Share your thoughts', gradient: 'from-royal-purple to-hot-pink' },
          { to: '/admin/tasks', icon: Plus, title: 'Add New Task', desc: 'Stay organized', gradient: 'from-hot-pink to-rose-pink' },
          { to: '/admin/ai-agent', icon: Bot, title: 'Chat with AI', desc: 'Your personal assistant', gradient: 'from-deep-purple to-royal-purple' },
        ].map((action, index) => (
          <motion.div
            key={action.to}
            initial={{ opacity: 0, y: 20 }}
          ...
          >
            <Link to={action.to}>
              <Card className="border-0 shadow-card cursor-pointer h-full card-hover">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {['hero', 'about', 'skills', 'experience', 'projects', 'contact'].map((section) => (
                    <div key={section} className="flex items-center justify-between p-3 rounded-lg bg-gradient-to-r from-purple-50 to-pink-50">
            ...
            <CardContent className="p-6">
              <div className="space-y-4">
                {portfolio?.skills?.map((skill, index) => (
                  <motion.div 
                    key={index} 
                    className="flex items-center gap-4 p-4 rounded-lg bg-gradient-to-r from-purple-50 to-pink-50"
                    initial={{ opacity: 0, x: -20 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: index * 0.05 }}
                  >
                    ...
                    <Input
                      ...
                      className="w-32 border-purple-200"
                ...
                <Button
                  ...
                  className="w-full border-dashed border-purple-300"
                >
            ...
            <CardContent className="p-6">
              <div className="space-y-4">
                {portfolio?.experience?.map((exp, index) => (
                  <motion.div 
                    key={exp.id || index} 
                    className="p-4 rounded-lg bg-gradient-to-r from-purple-50 to-pink-50 space-y-3"
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ delay: index * 0.1 }}
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-medium text-royal-purple">Experience {index + 1}</span>
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => {
            ...
            <CardContent className="p-6">
              <div className="space-y-4">
                {portfolio?.projects?.map((project, index) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-medium text-royal-purple">Project {index + 1}</span>
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => {
                ...
                {[
                  ...
                  <div key={field.key}>
                    ...
                    <Input
                      ...
                      onChange={(e) => updateField('contact', { ...portfolio?.contact, [field.key]: e.target.value })}
      ...
      <div className="lg:col-span-2">
        <Card className="border-0 shadow-card h-full flex flex-col overflow-hidden">
          <CardHeader className="gradient-bg text-white">
            <div className="flex items-center gap-3">
              <motion.div className="w-12 h-12 rounded-xl bg-white/20 flex items-center justify-center" animate={{ boxShadow: ["0 0 10px rgba(255,255,255,0.3)", "0 0 20px rgba(255,255,255,0.5)", "0 0 10px rgba(255,255,255,0.3)"] }} transition={{ duration: 2, repeat: Infinity }}>
                <Bot className="w-6 h-6 text-white" />
              </motion.div>
              <div>
                <CardTitle className="text-white">AI Personal Assistant</CardTitle>
                <CardDescription className="text-white/70">Your intelligent helper with memory</CardDescription>
              </div>
            </div>
          </CardHeader>
            ...
            <div className="space-y-4">
              {messages.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-muted-foreground max-w-md mx-auto">I can help you manage tasks, remember important things, and provide productivity suggestions.</p>
                </div>
              )}
              {messages.map((msg, index) => (
                <motion.div key={index} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={\`flex \${msg.role === 'user' ? 'justify-end' : 'justify-start'}\`}>
                  <div className={\`max-w-[80%] p-4 rounded-2xl \${msg.role === 'user' ? 'gradient-bg text-white rounded-br-sm' : 'bg-gradient-to-r from-purple-50 to-pink-50 rounded-bl-sm border border-purple-100'}\`}>
              ...
              {loading && (
                <div className="flex justify-start">
                  <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-4 rounded-2xl rounded-bl-sm border border-purple-100">
                    <div className="flex gap-1">
                      {[0, 1, 2].map(i => (<motion.div key={i} animate={{ y: [0, -5, 0] }} transition={{ duration: 0.5, repeat: Infinity, delay: i * 0.1 }} className="w-2 h-2 rounded-full bg-royal-purple" />))}
                    </div>
                  </div>
                </div>
              )}
          ...
          <div className="p-4 border-t bg-gradient-to-r from-purple-50/50 to-pink-50/50">
            <form onSubmit={(e) => { e.preventDefault(); sendMessage(); }} className="flex gap-3">
              <Input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask me anything..." className="flex-1 border-purple-200" disabled={loading} />
              <Button type="submit" className="gradient-bg text-white" disabled={loading}><Send className="w-4 h-4" /></Button>
            </form>
          </div>
        </Card>
      </div>
        ...
        <Card className="border-0 shadow-card h-full flex flex-col">
          <CardHeader className="border-b border-purple-100">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" onClick={clearMemory} className="text-destructive hover:bg-red-50"><Trash2 className="w-4 h-4 mr-1" />Clear</Button>
            </div>
          </CardHeader>
          <ScrollArea className="flex-1">
            <div className="p-4 space-y-3">
              {memories.length === 0 ? (<p className="text-sm text-muted-foreground text-center py-8">No memories stored yet</p>) : (
                memories.slice(0, 10).map((memory) => (
                  <motion.div key={memory.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="p-3 rounded-lg bg-gradient-to-r from-purple-50 to-pink-50 text-sm">
                    <p className="line-clamp-3">{memory.content}</p>
                    <p className="text-xs text-muted-foreground mt-2">{new Date(memory.created_at).toLocaleDateString()}</p>
                  </motion.div>
                ))
              )}
            </div>
...
const TasksPage = () => {
  ...
  const fetchTasks = async () => {
    try { const data = await api.get('/tasks', token); setTasks(data); } catch (error) { console.error('Error fetching tasks:', error); } finally { setLoading(false); }
  };

  const addTask = async () => {
    if (!newTask.title) return;
    try { const response = await api.post('/tasks', newTask, token); setTasks([...tasks, response.task]); setNewTask({ title: '', description: '', priority: 'medium', deadline: '' }); setShowAddTask(false); } catch (error) { console.error('Error adding task:', error); }
  };

  const toggleTask = async (taskId, completed) => {
    try { await api.put(\`/tasks/\${taskId}\`, { completed: !completed }, token); setTasks(tasks.map(t => t.id === taskId ? { ...t, completed: !completed } : t)); } catch (error) { console.error('Error updating task:', error); }
  };

  const deleteTask = async (taskId) => {
    try { await api.delete(\`/tasks/\${taskId}\`, token); setTasks(tasks.filter(t => t.id !== taskId)); } catch (error) { console.error('Error deleting task:', error); }
  };
  ...
  return (
    <motion.div variants={pageVariants} initial="initial" animate="animate" exit="exit" className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h2 className="text-2xl font-display font-bold">Tasks & Reminders</h2><p className="text-muted-foreground">Stay organized and productive</p></div>
        <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}><Button onClick={() => setShowAddTask(true)} className="gradient-bg text-white"><Plus className="w-4 h-4 mr-2" />Add Task</Button></motion.div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[{ label: 'Total Tasks', value: tasks.length, icon: Calendar, color: 'from-royal-purple to-hot-pink' }, { label: 'Completed', value: tasks.filter(t => t.completed).length, icon: Check, color: 'from-green-400 to-emerald-500' }, { label: 'Pending', value: tasks.filter(t => !t.completed).length, icon: Clock, color: 'from-hot-pink to-rose-pink' }].map((stat, i) => (
      ...
      <Card className="border-0 shadow-card">
        <CardContent className="p-6">
          {loading ? (<div className="space-y-3">{[1, 2, 3].map(i => <Skeleton key={i} className="h-20 w-full" />)}</div>) : tasks.length === 0 ? (
            <div className="text-center py-12"><Calendar className="w-16 h-16 mx-auto mb-4 text-muted-foreground" /><h3 className="font-semibold mb-2">No Tasks Yet</h3><p className="text-sm text-muted-foreground">Create your first task to get started</p></div>
          ) : (
            <div className="space-y-3">
              {tasks.map((task, index) => (
                <motion.div key={task.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.05 }} className={\`flex items-center gap-4 p-4 rounded-xl border \${task.completed ? 'bg-gray-50 opacity-60' : 'bg-gradient-to-r from-white to-purple-50/50'}\`}>
                  <button onClick={() => toggleTask(task.id, task.completed)} className={\`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors \${task.completed ? 'bg-green-500 border-green-500' : 'border-purple-300 hover:border-royal-purple'}\`}>{task.completed && <Check className="w-4 h-4 text-white" />}</button>
                  <div className="flex-1"><p className={\`font-medium \${task.completed ? 'line-through' : ''}\`}>{task.title}</p>{task.description && <p className="text-sm text-muted-foreground">{task.description}</p>}{task.deadline && <p className="text-xs text-muted-foreground mt-1 flex items-center gap-1"><Clock className="w-3 h-3" />{new Date(task.deadline).toLocaleDateString()}</p>}</div>
                  <Badge className={\`\${priorityColors[task.priority]} border\`}>{task.priority}</Badge>
                  <Button variant="ghost" size="icon" onClick={() => deleteTask(task.id)}><Trash2 className="w-4 h-4 text-destructive" /></Button>
                </motion.div>
              ))}
            </div>
          )}
      ...
      <Dialog open={showAddTask} onOpenChange={setShowAddTask}>
        <DialogContent className="border-0 shadow-2xl">
          <DialogHeader><DialogTitle>Add New Task</DialogTitle><DialogDescription>Create a new task to stay organized</DialogDescription></DialogHeader>
          <div className="space-y-4">
...
const WritingStudioPage = () => {
  ...
  const fetchArticles = async () => { try { const data = await api.get('/articles', token); setArticles(data); } catch (error) { console.error('Error fetching articles:', error); } finally { setLoading(false); } };

  const saveArticle = async (publish = false) => {
    if (!currentArticle.title || !currentArticle.content) return;
    setSaving(true);
    try {
      if (editing) { await api.put(\`/articles/\${editing}\`, { ...currentArticle, published: publish }, token); setArticles(articles.map(a => a.id === editing ? { ...a, ...currentArticle, published: publish } : a)); }
      else { const response = await api.post('/articles', { ...currentArticle, published: publish }, token); setArticles([response.article, ...articles]); }
      setCurrentArticle({ title: '', content: '', excerpt: '', cover_image: '' }); setEditing(null);
    } catch (error) { console.error('Error saving article:', error); } finally { setSaving(false); }
  };

  const deleteArticle = async (articleId) => { try { await api.delete(\`/articles/\${articleId}\`, token); setArticles(articles.filter(a => a.id !== articleId)); } catch (error) { console.error('Error deleting article:', error); } };
  ...
  return (
    <motion.div variants={pageVariants} initial="initial" animate="animate" exit="exit" className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <Card className="border-0 shadow-card">
            <CardHeader><CardTitle>{editing ? 'Edit Article' : 'New Article'}</CardTitle></CardHeader>
...
const GalleryManagerPage = () => {
  ...
  useEffect(() => { fetchPhotos(); }, [token]);

  const fetchPhotos = async () => { try { const data = await api.get('/gallery', token); setPhotos(data); } catch (error) { console.error('Error fetching gallery:', error); } finally { setLoading(false); } };

  const toggleVisibility = async (photoId, visible) => { try { await api.put(\`/gallery/\${photoId}\`, { visible: !visible }, token); setPhotos(photos.map(p => p.id === photoId ? { ...p, visible: !visible } : p)); } catch (error) { console.error('Error updating photo:', error); } };

  const updateCaption = async (photoId, caption) => { try { await api.put(\`/gallery/\${photoId}\`, { caption }, token); setPhotos(photos.map(p => p.id === photoId ? { ...p, caption } : p)); } catch (error) { console.error('Error updating caption:', error); } };
  ...
  return (
    <motion.div variants={pageVariants} initial="initial" animate="animate" exit="exit" className="space-y-6 max-w-2xl">
      ...
      <Card className="border-0 shadow-card"><CardHeader><CardTitle>Account</CardTitle><CardDescription>Manage your admin account</CardDescription></CardHeader><CardContent>
        <div className="flex items-center gap-4">
          <Avatar className="w-16 h-16 border-2 border-royal-purple/20"><AvatarImage src={PROFILE_PHOTO} /><AvatarFallback className="gradient-bg text-white text-xl">MA</AvatarFallback></Avatar>
          <div><p className="font-semibold">MiryamAbida07</p><p className="text-sm text-muted-foreground">Administrator</p></div>
        </div>
      </CardContent></Card>
    </motion.div>
  );
